...
- 상태: **구현·검증 완료 + 심화 확장** (필수 13/13, 보너스 4/4, 테스트 200개, 추가 기능 3종: 별점–감정 불일치 분석 · 속성별 감정 분석(ABSA) · 병렬 호출/비용 추적)
...
- [x] **10단계** 테스트 도입(pytest 168개) + `mismatch` 커맨드 → `bd4c01f`
- [x] **11단계** 속성별 감정 분석(ABSA) — `aspects` 커맨드, 차트·리포트 통합, 테스트 200개 → `fa20250`
- [x] **12단계** 병렬 호출 최적화 + 토큰·비용 추적 — `usage` 커맨드, 테스트 236개
...
2. 저장소 위치를 `~/Desktop/codyssey/A2-3` 로 결정(처음엔 A2-2 저장소 안에 폴더가 생성돼 중첩 repo가 될 뻔했음). `git init` 완료.
...
### 2026-07-28 (심화 확장: 테스트 · 불일치 분석 · ABSA)

16. **테스트 200개 도입(pytest)**. 순수 함수와 임시 SQLite 만 써서 **API 키·네트워크 없이 전부 실행**된다.
    경계·이상값 중심 — 별점 0·7점, 해석 불가한 날짜, 모델이 목록에 없는 속성을 지어내는 경우,
    재분석 시 예전 속성이 유령으로 남는 경우 등.
17. **`mismatch` 커맨드 신규**: 품질 지표의 별점–감정 일치율 90%에서, 남은 10%가 무엇인지 파고든 결과.
    실데이터에서 **불일치 10건이 전부 "숨은 불만"(별점은 후한데 본문은 부정) 한쪽 방향**이고,
    ★1·★2·★5는 불일치 0%인데 **★3(35.7%)·★4(21.7%)에만 몰려** 있었다.
    → "별점은 후하게 주면서 본문으로만 불만을 남기는" 패턴. 별점 평균만 추적하면 통째로 놓친다.
18. **`aspects` 커맨드 신규 (ABSA)**: `"용량이 작아서 1인분만 가능합니다. 혼자 살면 충분해요"`(★4→중립)
    같은 리뷰가 전체 감정 하나로 뭉개지는 것을 보고 추가. 배송·품질·가격·고객서비스·사용성·디자인 6종.
    실호출 결과(95건 → 속성 109건):
    ...
    전체 긍정 비율은 44.7%인데 속성별로 보면 **문제가 고객서비스·배송에만 몰려 있다**.
    전체 감정 하나로는 이 편중이 평균에 묻힌다.
19. **개선 우선순위 산식 결함을 발견해 수정**: 처음엔 부정 '건수'로만 정렬해서,
    8건 중 7건(87.5%) 부정인 고객서비스가 순위에서 빠지고 39.4%인 품질이 1위로 나왔다.
    → `부정 건수 × 부정률`로 바꿔 영향 범위와 심각도를 함께 반영. 배송 → 고객서비스 → 품질 순으로 정정됨.

### 2026-07-28 (병렬 호출 최적화 · 비용 추적)

20. **병렬 호출로 전환**. 리뷰당 1회 호출이라 건수에 비례해 느렸는데(103건 ABSA 에 수 분),
    시간의 대부분이 응답 대기라 CPU 는 놀고 있었다. `ThreadPoolExecutor` 로 겹쳐 부르되
    **DB 쓰기는 주 스레드에서** 처리 — `sqlite3` 는 스레드 공유가 막혀 있고, 굳이 풀어도
    병목은 네트워크지 DB 가 아니라 이득이 없다.
...
21. **`--mock` 은 스레드를 띄우지 않도록** 했다. 네트워크를 타지 않아 이득이 없고,
    순차 실행이라야 출력 순서가 재현되어 디버깅에 쓸 수 있다.
22. **토큰·비용 추적 추가**. 응답의 `usage` 를 락으로 보호하며 누적해(병렬 호출 대비)
    `api_usage` 테이블에 실행별로 남기고, `usage` 커맨드와 대시보드 리포트에 노출.
    실측: 103건 속성 분석에 33,539토큰 / 약 $0.0071.
23. **단가를 모르는 모델의 비용을 0 이 아닌 `None`("가격 정보 없음")으로** 두기로 했다.
    0 으로 표시하면 공짜로 썼다는 잘못된 인상을 준다. 단가표 자체도 `config.ai.pricing` 으로
    외부화 — 공급자 가격은 수시로 바뀌어 코드에 박아두면 곧 틀린 값이 된다.
...
| 2026-07-28 | 테스트를 순수 함수 위주로 설계(API 모킹 최소화) | 키·네트워크 없이 돌아가야 어디서든 재현 가능. 실제 회귀가 잘 나는 곳도 파싱·정규화 계층이다 |
| 2026-07-28 | 속성 목록을 `config.absa.aspects` 로 외부화 | 도메인이 바뀌면(음식점·앱 등) 코드가 아니라 설정만 고치면 되도록 |
| 2026-07-28 | 속성 재분석을 UPSERT 가 아닌 삭제 후 삽입으로 | UPSERT 면 이번 분석에서 안 나온 속성이 예전 값 그대로 남아 유령 데이터가 됨 |
| 2026-07-28 | 모델이 답한 속성을 화이트리스트로 검증 | 목록에 없는 속성을 지어내는 경우가 실제로 있어 집계가 오염됨 |
| 2026-07-28 | 개선 우선순위를 `부정 건수 × 부정률` 로 | 건수만 보면 언급 많은 속성이, 비율만 보면 표본 작은 속성이 과대평가됨 (실데이터에서 실제로 순위가 뒤바뀌는 것을 확인) |
| 2026-07-28 | API 호출은 병렬, DB 쓰기는 주 스레드 | sqlite3 는 스레드 공유가 막혀 있고, 병목은 네트워크지 DB 가 아님 |
| 2026-07-28 | `--mock` 과 `--workers 1` 은 스레드 없이 순차 | 재현 가능한 출력 순서가 필요한 경우(디버깅·테스트)를 남겨둠 |
| 2026-07-28 | 단가표를 `config.ai.pricing` 으로 외부화 | 공급자 가격은 수시로 바뀌므로 코드에 박으면 곧 틀린 값이 됨 |
| 2026-07-28 | 모르는 모델의 비용은 0 이 아닌 `None` | 0 은 "공짜로 썼다"는 잘못된 인상을 줌 |
...
.venv/bin/python main.py aspects --unanalyzed                     # 속성별 감정(ABSA)
.venv/bin/python main.py mismatch                                 # 별점–감정 불일치
.venv/bin/python main.py usage                                    # 토큰·비용 조회
.venv/bin/python main.py dashboard --html                         # 차트 + 리포트 + HTML
.venv/bin/pytest -q                                               # 테스트 236개
```
